...
def get_title_property_as_string(page: dict, prop_name: str) -> str:
    ...
    return "".join(t.get("plain_text", "") for t in prop["title"]) or
"(Untitled)"
...
if st.button("Find templates"):
    if not project_name.strip():
        ...
        if not project_page_id:
            ...
            st.success(f'Found {len(rows)} template row(s) for
"{project_name}".')

            for row in rows:
                st.write("-", get_title_property_as_string(row,
TITLE_PROPERTY_NAME))


if st.button("Trash templates"):
    rows = st.session_state.get("template_rows", [])
    saved_project_name = st.session_state.get("project_name",
project_name.strip())
    ...
    if not rows:
        ...
        st.success(f'Trashed {trashed_count} template row(s) for
"{saved_project_name}".')
        st.session_state["template_rows"] = []

On Wed, Apr 29, 2026 at 7:34 PM Samuel Roman <samroman@umich.edu> wrote:
>
>
